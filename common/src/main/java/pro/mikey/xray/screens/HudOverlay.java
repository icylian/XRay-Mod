...
public class HudOverlay {
    ...
    private static void renderDensityCompass(GuiGraphicsExtractor graphics) {
        ...
        if (!facing.isEmpty()) {
            graphics.text(font, facing, (screenWidth - font.width(facing)) / 2, y - 10, 0xFF66E6FF);
        }

        if (density.here() > 0) {
            String line = I18n.get("xray.compass.here", density.here());
            graphics.text(font, line, (screenWidth - font.width(line)) / 2, y + 40, 0xFFFFAA00);
        }
    }
    ...
    private static void text(GuiGraphicsExtractor graphics, Font font, String key, int count, int screenWidth, int y, int max, boolean isFacing) {
        int color = count > 0 ? (0xFF000000 | lerpColor(count, max)) : 0xFF555555;
        String line = (isFacing ? "▶ " : "") + I18n.get(key, count);
        graphics.text(font, line, (screenWidth - font.width(line)) / 2, y, isFacing ? 0xFF66E6FF : color);
    }
